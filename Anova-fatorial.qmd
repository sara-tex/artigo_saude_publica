...
df <- mutate(df, ID= 1:nrow(df), .before= 1)
```
...
modelo <- aov_ez(id = "ID",dv = "prop_psi_muni", 
                 between = c("nivel_atencao", "regiao"), data = df)
...
normalidade_res <- shapiro_test(residuos) 

# Avaliando homogeneidade de variância
homogeneidade <- levene_test(FS ~ Estciv*Sexo, data = df)
...
marginal_means <- bootstrap_model(modelo$lm, iterations = 1000) %>% 
  emmeans( ~ Estciv*Sexo)

# Efeitos condicionais para Sexo
contr_cond_sexo <- marginal_means %>% 
  contrast(method = "pairwise", by = "Sexo") %>% 
  parameters(bootstrap = TRUE, ci_method = "bci")

# Efeitos condicionais para Estado civil
contr_cond_estciv <- marginal_means %>% 
  contrast(method = "pairwise", by = "Estciv") %>% 
  parameters(bootstrap = TRUE, ci_method = "bci")
...
d_cond_sexo <- df %>% 
  group_by(Sexo) %>% 
  cohens_d(FS ~ Estciv, var.equal = F, ci = T, ci.type = "bca", nboot = 1000)
